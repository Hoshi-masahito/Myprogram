...
def load_datasets():
	...
	trainset = torchvision.datasets.ImageFolder(root="sample_data/",transform=transform)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(64, 32, 2, 2, 0), #32x32
            nn.BatchNorm2d(32),
            nn.ReLU(),

            nn.ConvTranspose2d(32, 3, 2, 2, 0),
            nn.Tanh()
        )
...
def train():
    ...
    for i in range(300):
        ...
        # 画像を保存
        if not os.path.exists("2d_generated"):
            os.mkdir("2d_generated")
        torchvision.utils.save_image(fake_img_tensor[:min(batch_len, 100)],
                                f"2d_generated/epoch_{i:03}.png", normalize = True)
    # ログの保存
    with open("2d_generated/logs.pkl", "wb") as fp:
